...
# Make a directory for uploads if it doesn't exist
script_dir = os.path.dirname(os.path.abspath(__file__))

# Build full path to the pkl files
model_path = os.path.join(script_dir, 'optimized_soil_carbon_model.pkl')
selector_path = os.path.join(script_dir, 'feature_selector.pkl')

# Load the files using full path
...
# Allowed extensions
ALLOWED_EXTENSIONS_HDR = {'hdr'}
ALLOWED_EXTENSIONS_BIN = {'bin'}
...
def index():
    ...
    if request.method == 'POST':
        ...
        # Check extensions
        if not (allowed_file(hdr_file.filename, ALLOWED_EXTENSIONS_HDR) and
                allowed_file(bin_file.filename, ALLOWED_EXTENSIONS_BIN)):
            error = "Invalid file type! Please upload a valid .hdr file and .bin file."
            return render_template('index.html', rows=rows, cols=cols, value=value, error=error)
        ...
        try:
            ...
            # Clear previous map and dimensions
            org_c_map = None
            rows, cols = 0, 0
...
def get_value():
    ...
    return jsonify({'value': f"{value:.3f} %" if value is not None else "N/A"})
